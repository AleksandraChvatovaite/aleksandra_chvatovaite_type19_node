...
shopItemRouter.get('/shop_items/itemId/:itemId', async (req, res) => {
...
shopItemRouter.delete('/shop_items/itemId/:itemId', async (req, res) => {
  const { itemId } = req.params;
  const sql = `UPDATE ${tableName} SET isArchived=1 WHERE shop_item_id=? LIMIT 1`;
